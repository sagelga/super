...
test.describe("Home Page", () => {
    ...
    test("should have a visible hero section", async ({ page }) => {
        const heroSection = page
            .locator('section, main > div, [class*="hero"]')
            .first();
    ...
    test("should have visible skills section", async ({ page }) => {
        ...
        const skillsSection = page
            .locator("h1, h2, h3")
            .filter({ hasText: /skills|skill/i })
            .first();
        if (
            await skillsSection.isVisible({ timeout: 3000 }).catch(() => false)
        ) {
...
test.describe("mobile home layout", () => {
    ...
    test("hero section renders on mobile without overflow", async ({
        page,
    }) => {
        await page.goto("/en");
        await waitForPageLoad(page);
