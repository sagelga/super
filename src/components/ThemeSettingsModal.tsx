"use client";

import React from "react";
import BottomSheet from "./ui/BottomSheet";
import SettingsOptionList, { SettingsOption } from "./ui/SettingsOptionList";
import { useTheme } from "../hooks/useTheme";
import { useTranslations } from "next-intl";

interface ThemeSettingsModalProps {
    isOpen: boolean;
    onClose: () => void;
}

const ThemeSettingsModal: React.FC<ThemeSettingsModalProps> = ({
    isOpen,
    onClose,
}) => {
    const { theme, setTheme } = useTheme();
    const t = useTranslations("common");

    const handleSelect = (value: "light" | "dark" | "system") => {
        setTheme(value);
        onClose();
    };

    const checkmarkIcon = (
        <svg
            width="20"
            height="20"
            viewBox="0 0 24 24"
            fill="none"
            stroke="currentColor"
            strokeWidth="2.5"
            strokeLinecap="round"
            strokeLinejoin="round"
        >
            <polyline points="20 6 9 17 4 12" />
        </svg>
    );

    const sunIcon = (
        <svg
            width="24"
            height="24"
            viewBox="0 0 24 24"
            fill="none"
            stroke="currentColor"
            strokeWidth="2"
            strokeLinecap="round"
            strokeLinejoin="round"
        >
            <circle cx="12" cy="12" r="5" />
            <line x1="12" y1="1" x2="12" y2="3" />
            <line x1="12" y1="21" x2="12" y2="23" />
            <line x1="4.22" y1="4.22" x2="5.64" y2="5.64" />
            <line x1="18.36" y1="18.36" x2="19.78" y2="19.78" />
            <line x1="1" y1="12" x2="3" y2="12" />
            <line x1="21" y1="12" x2="23" y2="12" />
            <line x1="4.22" y1="19.78" x2="5.64" y2="18.36" />
            <line x1="18.36" y1="5.64" x2="19.78" y2="4.22" />
        </svg>
    );

    const moonIcon = (
        <svg
            width="24"
            height="24"
            viewBox="0 0 24 24"
            fill="none"
            stroke="currentColor"
            strokeWidth="2"
            strokeLinecap="round"
            strokeLinejoin="round"
        >
            <path d="M21 12.79A9 9 0 1 1 11.21 3 7 7 0 0 0 21 12.79z" />
        </svg>
    );

    const monitorIcon = (
        <svg
            width="24"
            height="24"
            viewBox="0 0 24 24"
            fill="none"
            stroke="currentColor"
            strokeWidth="2"
            strokeLinecap="round"
            strokeLinejoin="round"
        >
            <rect x="2" y="3" width="20" height="14" rx="2" ry="2" />
            <line x1="8" y1="21" x2="16" y2="21" />
            <line x1="12" y1="17" x2="12" y2="21" />
        </svg>
    );

    const options: SettingsOption[] = [
        {
            id: "light",
            label: t("theme.light"),
            description: t("theme.light_description"),
            leading: sunIcon,
            trailing: theme === "light" ? checkmarkIcon : undefined,
            isActive: theme === "light",
            onClick: () => handleSelect("light"),
        },
        {
            id: "dark",
            label: t("theme.dark"),
            description: t("theme.dark_description"),
            leading: moonIcon,
            trailing: theme === "dark" ? checkmarkIcon : undefined,
            isActive: theme === "dark",
            onClick: () => handleSelect("dark"),
        },
        {
            id: "system",
            label: t("theme.system"),
            description: t("theme.system_description"),
            leading: monitorIcon,
            trailing: theme === "system" ? checkmarkIcon : undefined,
            isActive: theme === "system",
            onClick: () => handleSelect("system"),
        },
    ];

    return (
        <BottomSheet isOpen={isOpen} onClose={onClose} title={t("theme.title")}>
            <SettingsOptionList options={options} />
        </BottomSheet>
    );
};

export default ThemeSettingsModal;
