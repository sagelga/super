...
export const ImageWithLoading: React.FC<ImageWithLoadingProps> = ({
    ...
    if (!src || hasError) {
        return (
            <div
                className={`flex animate-pulse items-center justify-center bg-surface text-muted ${placeholderClassName}`}
    ...
    return (
        <div className="relative" style={!fill ? { width, height } : undefined}>
            ...
            {isLoading && (
                <div
                    className={`absolute inset-0 animate-pulse bg-surface ${placeholderClassName}`}
                />
            )}
